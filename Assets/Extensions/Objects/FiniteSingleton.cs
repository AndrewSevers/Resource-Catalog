...
namespace Extensions {
  ...
  public class FiniteSingleton<T> : MonoBehaviour where T : MonoBehaviour {
    ...
    public static T Instance {
      get {
        if (_isClosing) {
          Debug.LogWarning("Application is closing [FinitieSingleton] had been destroyed and is no longer accessible");
          return null;
        }
          ...
          if (_instance == null) {
            ...
            if (_instances.Length > 0) {
              ...
              if (_instances.Length > 1) {
                Debug.LogError(string.Format("Application contains more than one (1) _instance of [Singleton] '{0}'. Reopen scene or stop creating multiple _instances to correct the issue", _instance));
              }
            } else {
