		...
		private bool DrawInspector(SerializedObject aObject) {
			...
			while (property.NextVisible(enterChildren)) {
				switch (property.name) {
					case "m_Script":
						EditorUtils.DisplayDisabledGroup(property, true, new GUILayoutOption[0]);
						enterChildren = false;
						break;
					case "currentState":
						EditorGUILayout.BeginHorizontal();

						int newIndex = EditorGUILayout.Popup(StringUtils.ToCamelCase(property.name), currentStateIndex, availableStates.ToArray());
