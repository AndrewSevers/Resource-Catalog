		...
		private void DrawGUI() {
			...
			if (selectedAnimator != null) {
				...
				if (inactiveObject != null) {
					GUILayout.Label(string.Format("The \"{0}\" object along the animator's path is disabled. Please enable it to continue.", inactiveObject.name));
				} else if (selectedAnimator.runtimeAnimatorController == null) {
					GUILayout.Label("The selected animator's controller is null.");
				} else {
		...
		private void DrawWindow(int aWindowID) {
			switch (aWindowID) {
				case 0: // Animator Window
					...
					GUILayout.Label(windows[aWindowID].ErrorMessage);
